...
> 🎯 目标：构建一个现代科技感的首页，带有导航栏、CTA 按钮、主题/语言切换

- [ ] 创建 `Header.tsx`，包含 Logo、登录按钮、语言切换、主题切换
- [ ] 完成 `Hero.tsx`：主标题、副标题、CTA
- [ ] 使用 Tailwind 完成响应式布局
- [ ] 引入 `next-themes` 完成暗黑模式
- [ ] 引入 `next-intl` 多语言切换（en / zh）

📚 学习内容：
- App Router 的 `layout.tsx` 使用方式
- 全局样式结构 + 页面布局
- Tailwind 响应式与样式管理
- 暗黑模式 + 多语言切换实现
